...
export class QuestionsListComponent {
  ...
  private getQuestionsByQuizId() {
    const quizId: string | null = this.getIdFromUrl();
    console.log('quizId ', quizId);

    if (quizId) {
      this.getUser().then(() => {
        this.getQuizToManageById(quizId).then(() => {
          if (
            this.currentLanguageId !== null &&
            this.currentLanguageId !== undefined
          ) {
            this.getQuestions(quizId, this.currentLanguageId);
          }
        });
      });
  ...
  private async getQuizToManageById(id: string): Promise<Quiz | null> {
  ...
  private async getQuestions(quizId: string, languageId: number) {
    console.log('getQuestions');
    if (quizId && languageId !== null && languageId !== undefined) {
      await this.getAllByQuizId(quizId, this.userAdmin, languageId);
    }
  }
